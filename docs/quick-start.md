...
这里是 EtherEngine 的快速入门指南，其目的是帮助您能够快速上手使用该接口库；故本文档只会对常用的接口内容进行介绍，许多重要的功能只会简单的介绍甚至未能提及。有关更多完整信息，您应阅读 [EtherEngine 完整手册](README.md)。
...
关于支持的 EtherEngine 内置模块名称，详见 [此处](README.md#模块索引)。
...
关于更多的图片渲染函数，详见 [完整手册：Graphic 模块](Graphic/README.md)
...
+ [算法模块](Algorithm/README.md)：封装了 数值限定、基础碰撞检测、色彩空间转换、Base64 编码解码 等常用算法
+ [压缩模块](Compress/README.md)：提供了 基于 zlib 库的二进制数据压缩和解压缩 算法
+ [网络模块](Network/README.md)：提供了 HTTP/HTTPS客户端和服务端创建、链接结构分隔 等操作
+ [时间模块](Time/README.md)：涵盖了 程序暂停、延时、动态延时、高分辨率计时 等功能
+ [XML 模块](XML/README.md)：提供了 XML 格式数据 解析 和 修改 等操作
+ [JSON 模块](JSON/README.md)：提供了 JSON 格式数据 编码 和 解码 等操作
+ [字符串模块](String/README.md)：为不同编码字符串相互转换提供了便捷操作
+ [操作系统模块](OS/README.md)：封装了 系统信息获取、平台相关文件系统 API 、剪切板操作 等功能
