...
#define MakeLMacro(identifier)  \
	{                           \
#identifier, identifier \
	}

#pragma region filedrop internals
#define LMap_Drop(value)    \
	{                       \
		value, value - 2792 \
	}
...
unordered_map<int, int> ModuleInteractivity::mapSingleEventList = {
	...
	{SDL_DROPCOMPLETE, EVENT_DROPFILE_READY}};
...
ModuleInteractivity::ModuleInteractivity()
{
	_vCMethods = {
		...
		{"GetDropfileList", getDropfileList}
	};
	...
	_vMacros = {
		...
		MakeLMacro(EVENT_DROPFILE_READY),
	};
}
...
static int getDropfileList(lua_State *l)
{
	for (char *s : dropList)
		lua_pushstring(l, s);
	dropList.clear();
	return (int)dropList.v.size();
}
